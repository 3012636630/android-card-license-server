...
public class LicenseActivity extends Activity {
  EditText cardInput, serverInput; TextView statusText; Button button;
  public void onCreate(Bundle b){ super.onCreate(b); requestWindowFeature(Window.FEATURE_NO_TITLE); buildUi(); cardInput.setText(getPreferences(0).getString("card","")); serverInput.setText(LicenseConfig.getBaseUrl(this)); button.setOnClickListener(new View.OnClickListener(){ public void onClick(View v){ activate(); }}); if(cardInput.getText().toString().trim().length()>0) heartbeat(); }
  void buildUi(){ ScrollView root=new ScrollView(this); LinearLayout outer=new LinearLayout(this); outer.setGravity(Gravity.CENTER); outer.setOrientation(LinearLayout.VERTICAL); outer.setPadding(dp(22),dp(22),dp(22),dp(22)); GradientDrawable bg=new GradientDrawable(GradientDrawable.Orientation.TOP_BOTTOM,new int[]{Color.rgb(14,18,35),Color.rgb(3,14,24)}); outer.setBackground(bg); LinearLayout box=new LinearLayout(this); box.setOrientation(LinearLayout.VERTICAL); box.setPadding(dp(20),dp(20),dp(20),dp(20)); GradientDrawable panel=new GradientDrawable(); panel.setColor(Color.rgb(22,29,47)); panel.setStroke(1,Color.rgb(49,68,90)); panel.setCornerRadius(dp(10)); box.setBackground(panel); TextView title=t("卡密验证",25,Color.rgb(243,255,249),true); TextView tip=t("输入后台生成的卡密，验证通过后进入应用。服务器地址可直接修改。",14,Color.rgb(159,178,197),false); cardInput=input("XXXX-XXXX-XXXX-XXXX",16); serverInput=input("验证后台地址",13); button=new Button(this); button.setText("验证并进入"); button.setTextColor(Color.rgb(6,18,15)); button.setTextSize(16); button.setAllCaps(false); GradientDrawable bb=new GradientDrawable(GradientDrawable.Orientation.LEFT_RIGHT,new int[]{Color.rgb(81,231,197),Color.rgb(255,238,97)}); bb.setCornerRadius(dp(10)); button.setBackground(bb); statusText=t("等待输入卡密",14,Color.rgb(215,255,245),false); box.addView(title); box.addView(tip); add(box,cardInput,22); add(box,serverInput,12); add(box,button,16); add(box,statusText,16); outer.addView(box,new LinearLayout.LayoutParams(-1,-2)); root.addView(outer); setContentView(root); }
  ...
  EditText input(String h,int sp){ EditText e=new EditText(this); e.setHint(h); e.setSingleLine(true); e.setTextColor(Color.WHITE); e.setHintTextColor(Color.rgb(120,144,156)); e.setTextSize(sp); e.setPadding(dp(14),0,dp(14),0); GradientDrawable d=new GradientDrawable(); d.setColor(Color.rgb(27,40,60)); d.setStroke(1,Color.rgb(48,72,99)); d.setCornerRadius(dp(10)); e.setBackground(d); return e; }
  void add(LinearLayout box, View v, int top){ LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(-1, v instanceof TextView ? -2 : dp(54)); lp.topMargin=dp(top); box.addView(v,lp); }
  int dp(int v){ return (int)(v*getResources().getDisplayMetrics().density+0.5f); }
  void activate(){ final String card=cardInput.getText().toString().trim(); if(card.length()==0){ statusText.setText("请输入卡密"); return; } LicenseConfig.saveBaseUrl(this, serverInput.getText().toString()); setLoading(true,"正在联网激活..."); new Thread(new Runnable(){ public void run(){ try { LicenseResult r=new LicenseClient(LicenseActivity.this).activate(card, deviceId(), LicenseConfig.APP_VERSION); if(r.ok){ getPreferences(0).edit().putString("card",card.toUpperCase()).apply(); runOnUiThread(new Runnable(){ public void run(){ enterMain(); }}); } else { final String msg=r.message; runOnUiThread(new Runnable(){ public void run(){ setLoading(false,"验证失败："+msg); }}); } } catch(final Exception e){ runOnUiThread(new Runnable(){ public void run(){ setLoading(false,"验证失败："+(e.getMessage()==null?"网络验证失败":e.getMessage())); }}); } }}).start(); }
  ...
  void setLoading(boolean l,String m){ button.setEnabled(!l); cardInput.setEnabled(!l); serverInput.setEnabled(!l); statusText.setText(m); }
