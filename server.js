...
public class LicenseActivity extends Activity {
  EditText cardInput; boolean loading=false;
  public void onCreate(Bundle b){ super.onCreate(b); requestWindowFeature(Window.FEATURE_NO_TITLE); getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE); buildUi(); cardInput.setText(getPreferences(0).getString("card","")); cardInput.setOnEditorActionListener(new TextView.OnEditorActionListener(){ public boolean onEditorAction(TextView v,int actionId,android.view.KeyEvent event){ if(actionId==EditorInfo.IME_ACTION_DONE || (event!=null && event.getKeyCode()==android.view.KeyEvent.KEYCODE_ENTER && event.getAction()==android.view.KeyEvent.ACTION_UP)){ activate(); return true; } return false; }}); cardInput.addTextChangedListener(new TextWatcher(){ public void beforeTextChanged(CharSequence s,int st,int c,int a){} public void onTextChanged(CharSequence s,int st,int b,int c){} public void afterTextChanged(Editable e){ String v=e.toString().trim(); if(!loading && v.length()>=19 && v.indexOf('-')>0) activate(); }}); if(cardInput.getText().toString().trim().length()>0) heartbeat(); }
  void buildUi(){ LinearLayout root=new LinearLayout(this); root.setGravity(Gravity.CENTER); root.setOrientation(LinearLayout.VERTICAL); root.setPadding(dp(20),0,dp(20),0); GradientDrawable bg=new GradientDrawable(GradientDrawable.Orientation.TOP_BOTTOM,new int[]{Color.rgb(10,14,28),Color.rgb(2,10,18)}); root.setBackground(bg); cardInput=input("请输入卡密",18); int w=getResources().getDisplayMetrics().widthPixels - dp(40); if(w>dp(430)) w=dp(430); if(w<dp(240)) w=dp(240); LinearLayout.LayoutParams lp=new LinearLayout.LayoutParams(w,dp(56)); root.addView(cardInput,lp); setContentView(root,new ViewGroup.LayoutParams(-1,-1)); }
  EditText input(String h,int sp){ EditText e=new EditText(this); e.setHint(h); e.setSingleLine(true); e.setImeOptions(EditorInfo.IME_ACTION_DONE); e.setInputType(android.text.InputType.TYPE_CLASS_TEXT | android.text.InputType.TYPE_TEXT_FLAG_CAP_CHARACTERS | android.text.InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD); e.setTextColor(Color.WHITE); e.setHintTextColor(Color.rgb(120,144,156)); e.setTextSize(sp); e.setGravity(Gravity.CENTER); e.setPadding(dp(14),0,dp(14),0); GradientDrawable d=new GradientDrawable(); d.setColor(Color.rgb(27,40,60)); d.setStroke(1,Color.rgb(48,72,99)); d.setCornerRadius(dp(10)); e.setBackground(d); return e; }
  int dp(int v){ return (int)(v*getResources().getDisplayMetrics().density+0.5f); }
  void activate(){ if(loading) return; final String card=cardInput.getText().toString().trim(); if(card.length()==0){ toast("请输入卡密"); return; } setLoading(true,null); new Thread(new Runnable(){ public void run(){ try { LicenseResult r=new LicenseClient(LicenseActivity.this).activate(card, deviceId(), LicenseConfig.APP_VERSION); if(r.ok){ getPreferences(0).edit().putString("card",card.toUpperCase()).apply(); runOnUiThread(new Runnable(){ public void run(){ enterMain(); }}); } else { final String msg=r.message; runOnUiThread(new Runnable(){ public void run(){ setLoading(false,"验证失败："+msg); }}); } } catch(final Exception e){ runOnUiThread(new Runnable(){ public void run(){ setLoading(false,"验证失败："+(e.getMessage()==null?"网络验证失败":e.getMessage())); }}); } }}).start(); }
